...
def get_true_stock(code, startdate, enddate):
    if enddate > inDBdate:
        ...
        if stock is None:
            return None
        if len(or_stock) < 90 + window + extension:
            return None
...
# outDF.to_pickle(data_dir + '\\2021verify5D.pkl')
# np.save(data_dir + '\\2021verifyX5D.npy', trainX)

outDF = pd.read_pickle(data_dir + '\\2021verify5D.pkl')
trainX = np.load(data_dir + '\\2021verifyX5D.npy')
